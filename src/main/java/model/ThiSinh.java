package model;

import java.io.Serializable;
import java.sql.Date;
import java.text.SimpleDateFormat;
import java.util.Objects;

public class ThiSinh implements Serializable {
	// serializable Ä‘á»ƒ dá»¯ liá»‡u xuá»‘ng táº­p tin

	private int maThiSinh;
	private String tenThiSinh;
	private Tinh queQuan;
	private Date ngaySinh;
	private boolean gioiTinh;
	private float diemMon1;
	private float diemMon2;
	private float diemMon3;
	private String lopHoc;

	public ThiSinh() {

	}

	public Object[] createObjects() {
		SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy");
		return new Object[] {getMaThiSinh(), getTenThiSinh(), getQueQuan().getTenTinh(), df.format(getNgaySinh()),
				(isGioiTinh() ? "Nam" : "Ná»¯"), getLopHoc(), getDiemTrungBinh(), getHocLuc() };
	}
	
	

	public ThiSinh(int maThiSinh, String tenThiSinh, Tinh queQuan, Date ngaySinh, boolean gioiTinh,
			float diemMon1, float diemMon2, float diemMon3, String lopHoc) {
		this.maThiSinh = maThiSinh;
		this.tenThiSinh = tenThiSinh;
		this.queQuan = queQuan;
		this.ngaySinh = ngaySinh;
		this.gioiTinh = gioiTinh;
		this.diemMon1 = diemMon1;
		this.diemMon2 = diemMon2;
		this.diemMon3 = diemMon3;
		this.lopHoc = lopHoc;
	}
	

	public int getMaThiSinh() {
		return maThiSinh;
	}

	public void setMaThiSinh(int maThiSinh) {
		this.maThiSinh = maThiSinh;
	}

	public String getTenThiSinh() {
		return tenThiSinh;
	}

	public void setTenThiSinh(String tenThiSinh) {
		this.tenThiSinh = tenThiSinh;
	}

	public Tinh getQueQuan() {
		return queQuan;
	}

	public void setQueQuan(Tinh queQuan) {
		this.queQuan = queQuan;
	}

	public Date getNgaySinh() {
		return ngaySinh;
	}

	public void setNgaySinh(Date ngaySinh) {
		this.ngaySinh = ngaySinh;
	}

	public boolean isGioiTinh() {
		return gioiTinh;
	}

	public void setGioiTinh(boolean gioiTinh) {
		this.gioiTinh = gioiTinh;
	}

	public float getDiemMon1() {
		return this.diemMon1;
	}

	public void setDiemMon1(float diemMon1) {
		this.diemMon1 = diemMon1;
	}

	public float getDiemMon2() {
		return this.diemMon2;
	}

	public void setDiemMon2(float diemMon2) {
		this.diemMon2 = diemMon2;
	}

	public float getDiemMon3() {
		return this.diemMon3;
	}

	public void setDiemMon3(float diemMon3) {
		this.diemMon3 = diemMon3;
	}
	
	public String getLopHoc() {
		return lopHoc;
	}

	public void setLopHoc(String lopHoc) {
		this.lopHoc = lopHoc;
	}

	public float getDiemTrungBinh() {
		return (this.diemMon1 + this.diemMon2 + this.diemMon3) / 3;
	}
	
	
	public String getHocLuc() {
		if(getDiemTrungBinh() >= 8 && getDiemTrungBinh() <= 10) {
			return "Giỏi";
		}else if(getDiemTrungBinh() >= 6.5 && getDiemTrungBinh() < 8) {
			return "Khá";
		}else if(getDiemTrungBinh() >= 3.5 && getDiemTrungBinh() < 6.5) {
			return "Trung Bình";
		}else if(getDiemTrungBinh() >= 0 && getDiemTrungBinh() < 3.5) {
			return "Yếu";
		}
		return null;
	}
	
	
	public String getThongTinThiSinh() {
		return maThiSinh + " | " + tenThiSinh + " | " + queQuan.getTenTinh()
				+ " | " + ngaySinh.toString() + " | " + (isGioiTinh()? "Nam" : "Nữ") 
				+ " | " + getLopHoc() + " | " + diemMon1 + " | " + diemMon2 + " | " + diemMon3 
				+ " | " + getDiemTrungBinh() + " | " + getHocLuc() + "\n";
	}

	@Override
	public int hashCode() {
		return Objects.hash(diemMon1, diemMon2, diemMon3, gioiTinh, lopHoc, maThiSinh, ngaySinh, queQuan, tenThiSinh);
	}

	@Override
	public boolean equals(Object obj) {
		if (this == obj)
			return true;
		if (obj == null)
			return false;
		if (getClass() != obj.getClass())
			return false;
		ThiSinh other = (ThiSinh) obj;
		return Float.floatToIntBits(diemMon1) == Float.floatToIntBits(other.diemMon1)
				&& Float.floatToIntBits(diemMon2) == Float.floatToIntBits(other.diemMon2)
				&& Float.floatToIntBits(diemMon3) == Float.floatToIntBits(other.diemMon3) && gioiTinh == other.gioiTinh
				&& Objects.equals(lopHoc, other.lopHoc) && maThiSinh == other.maThiSinh
				&& Objects.equals(ngaySinh, other.ngaySinh) && Objects.equals(queQuan, other.queQuan)
				&& Objects.equals(tenThiSinh, other.tenThiSinh);
	}

	@Override
	public String toString() {
		return "ThiSinh [maThiSinh=" + maThiSinh + ", tenThiSinh=" + tenThiSinh + ", queQuan=" + queQuan + ", ngaySinh="
				+ ngaySinh + ", gioiTinh=" + gioiTinh + ", diemMon1=" + diemMon1 + ", diemMon2=" + diemMon2
				+ ", diemMon3=" + diemMon3 + ", lopHoc=" + lopHoc + "]";
	}


	
	



}
